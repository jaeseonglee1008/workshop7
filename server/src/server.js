...
function getFeedItem(feedItemId,callback) {
 //get feeditem with the given ID
...
function getFeedData(user,callback) {
 ...
 db.collection('feeds').findOne({_id: userData.feed},
...
// Now, you can change the POST /feeditem route to use the new helper function. Notice how the logic is essentially the same, except we moved some logic into the callback argument of postStatusUpdate:
...
app.post('/feeditem/:feeditemid/comments', validate({ body: CommentSchema }), function(req, res) {
  ...
  if (fromUser === author) {
    ...
    console.log("error occurred");
          sendDatabaseError(res,err);
            ...
            console.log("success on the way");
          getFeedItem(feedItemid,function(err,feedItem){

              if(err){
                console.log("success didn't make it");
                sendDatabaseError(res,err);
                  }
              else{
                  ...
                  res.status(201);
                res.send(feedItem)
                //  console.log("feedItem contains"+feedItem)
                  }
            }
         );
      }
  ...
  app.put('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
      var fromUser = getUserIdFromToken(req.get('Authorization'));
      var userId = req.params.userid;
      var userIdHex =new ObjectID(userId);
      var feedItemid = new ObjectID(req.params.feeditemid);
      ...
      if (fromUser === userId) {
        ...
        db.collection('feedItems').updateOne({_id:feedItemid},
        {
          $addToSet:{ [`comments.${commentIdx}.likeCounter`]:userIdHex }
        },
        function(err){
          if(err) {
          return sendDatabaseError(res,err);
...
app.delete('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
  ...
  if (fromUser === userId) {
    ...
    db.collection('feedItems').updateOne({_id:feedItemid},
    {
      $pull:{[`comments.${commentIdx}.likeCounter`]:userIdHex}
    },
    function(err){
      if(err) {
      return sendDatabaseError(res,err);
